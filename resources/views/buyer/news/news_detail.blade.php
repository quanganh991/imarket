                ...
                <div class="left_content">
                    <div class="single_page">
                            ...
                            @if(Session::get('id_buyer'))
                                ...
                                @else
                                    <form action="{{URL::to('/unbookmark')}}" method="get">
                                        <input name="id_news" value="{{$newsDetail->id_news}}" type="hidden"/>
                                        <button style="background-color: red" class="btn default-btn">Bỏ lưu bài viết
                                        </button>
                                    </form>
                                @endif

                                <form action="{{URL::to('/chat-with-'.$newsDetail->id_user)}}" method="get">
                                    <input name="id_news" value="{{$newsDetail->id_news}}" type="hidden"/>
                                    <button style="background-color: #974194" class="btn default-btn">Chat với người
                                        bán
                                    </button>
                                </form>
                            @endif
                        ...
                        <br>
                            ...
                            <p style="color: red; font-size: 25px; font-weight: bold">Đánh giá:</p>
                            @if( $score_rated == 0)
                                <p style="color: deeppink; font-size: 25px;">Chưa được người dùng đánh giá</p>
                            @else
                                <p style="color: #74a743; font-size: 25px;">Đã có {{count($statistic)}} người dùng đánh giá</p>
                            @endif
                            <p style="color: #bd790e; font-size: 25px;">Trung bình: {{round($score_rated,2)}}</p>

                            <div class="col-lg-3">
                                <form action="{{URL::to('/rating')}}" method="GET">
                                    <input name="id_news" hidden value="{{$newsDetail->id_news}}"/>
                                    ...
                                    @if($user->status_user == 1)    <!--Nếu ko bị khóa tài khoản thì mới hiển thị bình luận-->
                                        ...
                                        @elseif($user->type_of_user == 2)
                                            <p style="color: red; font-size: 20px; font-weight: bold">
                                                Admin: {{$eachCommentList->name_user}}</p>
                                        @endif
                                        <p style="color: red; font-size: 15px; font-weight: bold"><i
                                                class="fa fa-comment"
                                        ...
                                        @if(Session::get('id_admin'))
                                            <a href="{{URL::to('/admin-delete-comment/'.$eachCommentList->id_coment)}}">Xóa
                                                comment</a>
                                            |
                                            <a href="{{URL::to('/admin-block-user/'.$eachCommentList->id_customer)}}">Khóa
                                                tài
                                                khoản</a>
                                        @endif
                                    </row>
                                    ...
                                    <li class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <form class="dropdown-item" action="{{URL::to('/reply')}}" method="GET">
                                                <textarea name="reply" placeholder="Bình luận" rows="6"
                                                          cols="50"></textarea>
                                                ...
                                                @if($userRep->status_user == 1 && $eachOfReply->is_valid_reply == 1)
                                                    @if($userRep->type_of_user != 2)
                                                        <p style="color: white; font-size: 20px; font-weight: bold"
                                                           class="dropdown-item">{{$userRep->name_user}}</p>
                                                    @elseif($userRep->type_of_user == 2)
                                                        <p style="color: yellow; font-size: 20px; font-weight: bold"
                                                           class="dropdown-item">Admin: {{$userRep->name_user}}</p>
                                                    @endif
                                                    ...
                                                    @if(Session::get('id_admin'))
                                                        ...
                                                        |
                                                        <a style="color: #e63084"
                                                           href="{{URL::to('/admin-block-user/'.$eachCommentList->id_customer)}}">Khóa
                                                            tài
                                                            khoản</a>
                                                    @endif
                                                @endif
                        ...
                        <!--Hết Bình luận-->
                            ...
                            <div class="related_post">
                                <p style=" font-size: 30px; font-weight: bold">Bài viết liên quan<i
                                        class="fa fa-thumbs-o-up"></i></p>
                        ...
                        <div class="single_sidebar">
                            ...
                            <div class="tab-content">
                                ...
                                <div role="tabpanel" class="tab-pane" id="comments">
                                    <!--Comment hàng đầu trong cùng branch-->
                                    <ul class="spost_nav">
                                        @foreach($relevantNewsDetail as $eachOfRelevantNewsDetail)
                                            <?php
